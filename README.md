...
传统 epoll 模型在高连接数场景下存在显著的性能损耗。ImDragonfly 基于 **Linux io\_uring** 实现零拷贝异步 IO，同时支持百万级并发连接。
...
> **性能说明**：
> - 测试环境：多核服务器环境下进行基准测试
> - DashTable 在插入、删除和内存占用方面均表现最优
> - 查找性能略逊于Redis dict，但在实际高并发场景下优势明显
> - 内存效率提升约 **38%**（相比Redis dict）
***
