 ...
 public:
  ...
  bool notify() noexcept {
    ...
    if (prev & kWaiterMask) {
      std::coroutine_handle<> handler;
      {
        ...
        if (!wait_queue_.NotifyOne(handler))
          return false;
      }
      // 在锁外恢复协程，避免协程内co_await EventCount死锁，见测试BlockingCounterTest.ReWaitAfterNotifyAllNoDeadlock
      handler.resume();
      return true;
    }
    return false;
  ...
  bool notifyAll() {
    ...
    if (prev & kWaiterMask) {
      ...
      {
        std::lock_guard lk(lock_);
        if (!wait_queue_.NotifyAll(handlers))
          return false;
      }
