...
DbSlice::ItAndUpdater DbSlice::FindMutable(const Context& cntx,
                                           std::string_view key) {
  return std::move(FindMutableInternal(cntx, key, std::nullopt).value());
}
DbSlice::ConstIterator DbSlice::FindReadOnly(const Context& cntx,
                                             std::string_view key) const {
  auto res = FindInternal(cntx, key, std::nullopt, UpdateStatsMode::kReadStats);
  return {*res, StringOrView::FromView(key)};
}
OpResult<DbSlice::ItAndUpdater> DbSlice::FindMutableInternal(
    const Context& cntx, std::string_view key,
    std::optional<unsigned> req_obj_type) {
  auto res =
      FindInternal(cntx, key, req_obj_type, UpdateStatsMode::kMutableStats);
  if (!res.ok()) {
    return res.status();
  }
...
auto DbSlice::FindInternal(const Context& cntx, std::string_view key,
                           std::optional<unsigned> req_obj_type,
  ...
  if (req_obj_type.has_value() &&
      it->second.ObjType() != req_obj_type.value()) {
...
facade::OpResult<DbSlice::ItAndUpdater> DbSlice::AddOrFind(
    const Context& cntx, std::string_view key,
    std::optional<unsigned> req_obj_type) {
  return AddOrFindInternal(cntx, key, req_obj_type);
}
facade::OpResult<DbSlice::ItAndUpdater> DbSlice::AddOrUpdate(
    const Context& cntx, std::string_view key, PrimeValue obj,
    uint64_t expire_at_ms) {
  return AddOrUpdateInternal(cntx, key, std::move(obj), expire_at_ms);
}

facade::OpResult<DbSlice::ItAndUpdater> DbSlice::AddNew(const Context& cntx,
                                                        std::string_view key,
                                                        PrimeValue obj,
                                                        uint64_t expire_at_ms) {
  ...
  return DbSlice::ItAndUpdater{
      res.it, AutoUpdater(cntx.GetDbIndex(), key, res.it, this),
      true};  // 注意一下
}

facade::OpResult<DbSlice::ItAndUpdater> DbSlice::AddOrFindInternal(
    const Context& cntx, std::string_view key,
    std::optional<unsigned> req_obj_type) {
  ...
  if (res.ok()) {
    ...
    if (res->IsOccupied()) {
      return ItAndUpdater{it, AutoUpdater(cntx.GetDbIndex(), key, it, this),
                          false};
    } else {
  ...
  try {
    ...
    LOG(WARNING) << "AddOrFindInternal OOM for key, db_index:" << cntx.GetDbIndex()
                 << " shard_id:" << shard_id_;
    return OpStatus::WRONG_TYPE;
  }
  ...
  return ItAndUpdater{
      ...
      AutoUpdater(cntx.GetDbIndex(), key,
                  Iterator(it, StringOrView::FromView(key)), this),
      true};
}

facade::OpResult<DbSlice::ItAndUpdater> DbSlice::AddOrUpdateInternal(
    const Context& cntx, std::string_view key, PrimeValue obj,
    uint64_t expire_at_ms) {
...
facade::OpResult<void> DbSlice::UpdateExpire(const Context& cntx,
                                             Iterator main_it, int64_t sec) {
  (void)cntx;
...
bool DbSlice::RemoveExpire(DbIndex db_ind, const Iterator& main_it) {
  ...
  if (!main_it->first.HasExpire()) return false;
...
DbSlice::Iterator DbSlice::ExpireIfNeeded(const Context& cntx,
                                          Iterator it) const {
  return Iterator::FromPrime(ExpireIfNeeded(cntx, it.GetInnerIt()));
}

PrimeIterator DbSlice::ExpireIfNeeded(const Context& cntx,
  ...
  const_cast<DbSlice*>(this)->PerformDeletionAtomic(
      Iterator(it, StringOrView::FromView(key)), db.get());
...
void DbSlice::PostUpdate(DbIndex /*db_ind*/, std::string_view /*key*/) {
  ...
  //     if (auto wit = watched_keys.find(std::string(key)); wit !=
  //     watched_keys.end()) {
...
void DbSlice::AutoUpdater::Cancel() { fields_ = {}; }
DbSlice::AutoUpdater::AutoUpdater(AutoUpdater&& o) noexcept {
  *this = std::move(o);
}
DbSlice::AutoUpdater& DbSlice::AutoUpdater::operator=(
    AutoUpdater&& o) noexcept {
  Run();
  fields_ = o.fields_;
  o.Cancel();
  return *this;
...
DbSlice::AutoUpdater::~AutoUpdater() { Run(); }
...
DbSlice::AutoUpdater::AutoUpdater(DbIndex db_ind, std::string_view key,
                                  const Iterator& it, DbSlice* db_slice)
