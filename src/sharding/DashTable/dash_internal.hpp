...
struct Segment {
  ...
  // 腾挪：把 from 桶里的一个条目搬到 to 桶，为插入腾出空位。
  //   own=true  → 搬走 from 自己的条目（非 probe 槽）
  //   own=false → 搬走 from 里"借宿"的条目（probe 槽，即上一个主桶的）
  // 返回被搬走的槽号；from 无可搬条目或 to 已满 → nullopt。
  ...
  // 插入核心：三级安置策略
  //
  // 第 1 级 均衡安置（spread=true）：
  //   home 与右邻 nxt 谁的空闲多，新条目优先落谁。
  //   "均衡" 让段内桶占用尽量平均，避免局部拥挤。
  //   落到 nxt 的条目标记为 probe 槽（借宿在邻居家）。
  //   spread=false（分裂搬入）则固定优先 home。
  //
  // 第 2 级 腾挪：
  //   主桶和邻居都满了，先挪出一个空位再插：
  //   a) 把 nxt 里一个"自家条目"再往后挪到 nxt+1（腾出 nxt 给 probe 槽）；
  //   b) 把 home 里一个"借宿条目"挪回它的左邻 home-1（腾出 home）。
  //   这相当于"挤一挤"，让条目在物理桶间小幅滑动。
  //
  // 第 3 级 溢出区：
  //   前两级都失败，发配到段尾溢出区（InsertIntoOverflow）。
  // ═══════════════════════════════════════════════════════════════
  template <typename K, typename V>
  ...
  // ═══════════════════════════════════════════════════════════════
  // 段分裂：local_depth_ + 1，逻辑上把段对半切。
  //
  // 判别位：新深度下的最高位——hash 第 (64 - local_depth_) 位：
  //   0 → 条目属于"左半"，留在本段；
  //   1 → 条目属于"右半"，搬进新段 dest（用 move，避免拷贝）。
  //
  // 溢出槽的处置：
  //   属于右半的 → 整体搬去 dest；
  //   属于左半的 → 尝试"回收"回主桶/邻居桶（TryReclaimOverflow）。
  //   分裂让桶数变多、拥挤缓解，被发配的条目有机会回归普通槽。
  // ═══════════════════════════════════════════════════════════════
  template <typename H>
