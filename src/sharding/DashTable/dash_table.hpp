...
class DashTable : public detail::DashTableBase {
 public:
  ...
  struct DefaultEvictionPolicy {
    ...
    bool CanGrow(const DashTable&) { return true; }
  ...
  explicit DashTable(
      size_t capacity_log = 1, const Policy& policy = Policy{},
      PMR_NS::memory_resource* mr = PMR_NS::get_default_resource());
  ~DashTable();
  template <typename U, typename V>
  std::pair<iterator, bool> Insert(U&& key, V&& value) {
  ...
  iterator InsertNew(U&& key, V&& value, EvictionPolicy&& ev) {
    return InsertInternal(std::forward<U>(key), std::forward<V>(value),
                          std::forward<EvictionPolicy>(ev),
 ...
 private:
  ...
  auto EqPred(const K& key) const {
    return [p = &policy_, &key](const auto& probe) -> bool {
      return p->Equal(probe, key);
    };
  }
  ...
  Policy policy_;
  std::vector<SegmentType*, PMR_NS::polymorphic_allocator<SegmentType*>>
      segment_;
};
...
class DashTable<_Key, _Value, Policy>::Iterator {
 ...
 public:
  ...
  bool is_done() const { return owner_ == nullptr; }
  ...
  friend bool operator==(const Iterator& lhs, const Iterator& rhs) {
    if (lhs.owner_ == nullptr && rhs.owner_ == nullptr) return true;
  ...
  auto buckets() const {
    ...
    return std::views::iota(0u, limit_) |  //   生成 0..limit_-1 的整数序列
           std::views::transform([*this, is_all](uint8_t i) {
             ...
             return bucket_iterator{owner_, seg_id_,
                                    index};  // 生成指向该桶的迭代器
           });
  }

  bool operator==(const BucketSet& other) const {
    return owner_ == other.owner_ && seg_id_ == other.seg_id_ &&
           limit_ == other.limit_ && ids_[0] == other.ids_[0] &&
           ids_[1] == other.ids_[1];
  }
...
DashTable表的创建
    ...
    术语:
        逻辑段:
segment_每一个元素为一个逻辑段，逻辑段为一个指针,指向物理段，不同的逻辑段可能指向相同的物理段
        物理段:
逻辑段为一个指针,指向真正存储数据的物理段，也就是dash_internal.hpp的Segment
    ...
    global_depth_要求global_depth_位哈希来在目录segment_找对应的逻辑段,
换种说法, key需要global_depth_位找对应的逻辑段 例子如下: capacity_log = 2;
        global_depth_ = 2;
        unique_segments_ = 4;
        目录(二进制表示):
...
*/
template <typename _Key, typename _Value, typename Policy>
DashTable<_Key, _Value, Policy>::DashTable(size_t capacity_log,
                                           const Policy& policy,
...
auto DashTable<_Key, _Value, Policy>::Find(U&& key) const -> const_iterator {
  ...
  uint32_t seg_id = SegmentId(
      key_hash);  // 使用 hash >> (64 - global_depth_)  高位作为 segment ID
  if (auto seg_it = segment_[seg_id]->FindIt(key_hash, EqPred(key));
      seg_it.found()) {
...
template <typename _Key, typename _Value, typename Policy>
template <typename Pred>
auto DashTable<_Key, _Value, Policy>::FindFirst(uint64_t key_hash, Pred&& pred)
    -> iterator {  // pred 精确比较键是否相等
...
DashTable<_Key, _Value, Policy>::ConstructSegment(uint8_t depth, uint32_t id) {
  ...
  /*
      ...
      11 -> 逻辑段4 -> 物理段4
      也就是local_depth_ =
     global_depth_的结果，如果物理段满了，就会扩大global_depth_和目录，因为没有多余的逻辑段进行操作
  */
...
void DashTable<_Key, _Value, Policy>::Clear() {
  ...
  if (global_depth_ > initial_depth_) {
    ...
    size_t dest = 0, src = 0;
    size_t new_size = (1 << initial_depth_);  // 全局目录，是initial_depth_,
                                              // 或者global_depth_的2次幂
    bucket_count_ = 0;
    while (
        src <
        segment_
            .size()) {  // 遍历每个段，从左往右遍历物理段，段数小于new_size，放入，否则摧毁
...
/*
    ...
    KEY寻找对应的逻辑段需要global_depth_位的哈希值，也就是key_hash = hash >> (64
   - global_depth_);
    对应的物理段就是key_hash的高local_depth_位，这里如果选择低位，处理段分裂不好处理，因为相同物理段的逻辑段的高位是相同的，具体看DashTable<_Key,
   _Value, Policy>::Split

    例子:
    global_depth_ = 3, 物理段的local_depth_都是 1, 那么物理段只要看hash >> (64 -
   global_depth_)的最高1位, 也就是0，1，只有两个物理段 如果local_depth_都是
   2，那么物理段只要看hash >> (64 - global_depth_)的最高2位, 也就是有4个物理段
    ;
    目录:
    000 -> 逻辑段1 -> 物理段1
    001 -> 逻辑段2 -> 物理段1
    ...
    为了区别他们，比如KEY的hash >> (64 -
   global_depth_)的最后1位是0,怎么区别是物理段1还是新分裂的物理段3呢
    答案是根据后面一位，比如逻辑段1 的 第二位是0，而逻辑段3
   第二位是3，就可以区分了，这里第二位是0是物理段1，是1是物理段3
...
auto DashTable<_Key, _Value, Policy>::InsertInternal(U&& key, V&& value,
  ...
  */
  ...
  while (true) {
    ...
    auto move_cb =
        [&](uint32_t segment_id, detail::PhysicalBid from,
            detail::PhysicalBid to) {  // 基本用于统计信息, 并不影响插入逻辑
          ev.OnMove(Cursor{global_depth_, segment_id, from},
                    Cursor{global_depth_, segment_id, to});
        };

    if (mode == InsertMode::kForceInsert) {
      it = target->InsertUniq(std::forward<U>(key), std::forward<V>(value),
                              key_hash, true, move_cb);
      res = it.found();
    } else {
      std::tie(it, res) =
          target->Insert(std::forward<U>(key), std::forward<V>(value), key_hash,
                         EqPred(key), move_cb);
    }
    ...
    if (res) {  // success
      ...
      ++size_;
      return std::make_pair(iterator{this, target_seg_id, it.index, it.slot},
                            true);
    }
    ...
    if (target->local_depth() == global_depth_) {
      // 这个 Segment 的 local_depth
      // 已经等于全局深度，意味着它在目录中只独占一个逻辑槽，没有多余的目录项可以用来映射即将分裂出的“兄弟段”
...
void DashTable<_Key, _Value, Policy>::Split(uint32_t seg_id,
  ...
  uint32_t chunk_size =
      1u << (global_depth_ - source->local_depth());  // 该段覆盖的目录项数量
  ...
  */
  ...
  source->Split(std::move(hash_fn), target,
                [&](uint32_t segment_from, detail::PhysicalBid from,
                    uint32_t segment_to, detail::PhysicalBid to) {
                  ev.OnMove(Cursor{global_depth_, segment_from, from},
                            Cursor{global_depth_, segment_to, to});
                });
  ++unique_segments_;
...
void DashTable<_Key, _Value, Policy>::IncreaseDepth(unsigned new_depth) {
  ...
  */
  ...
  for (int i = prev_sz - 1; i >= 0; --i) {  // 正序会错误覆盖
    ...
    std::fill(segment_.begin() + offs, segment_.begin() + offs + repl_cnt,
              segment_[i]);
...
void DashTable<_Key, _Value,
  ...
  if constexpr (IsSingleBucket) {
    ...
    while (seg_id_ < owner_->segment_.size()) {
      auto seg_it = owner_->segment_[seg_id_]->FindValidStartingFrom(bucket_id_,
                                                                     slot_id_);
      if (seg_it.found()) {
...
auto DashTable<_Key, _Value, Policy>::Traverse(Cursor curs, Cb&& cb) -> Cursor {
  ...
  if (bid >= Policy::kBucketNum || sid >= segment_.size()) return Cursor::end();
  ...
  do {
    ...
    if (sid >= segment_.size()) {
      ...
      if (bid >= Policy::kBucketNum) return Cursor::end();
    }
  } while (!fetched);
