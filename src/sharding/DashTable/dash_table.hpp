...
/*
 ...
 就需要扫描这个没扫描的物理段，以及这个物理段扩展出来的新段
 ，但是这不是最好的办法，会产生一直扫描不断产生的新段，SAVE事务一直结束不了的可能


 选择版本号递增就可以了，不需要多次SAVE调度，SAVE命令只要递增全局版本号(可能是哈希表版本号)，让桶发生事件时判断版本号同步写入持久化，
 桶2的数据向桶1腾挪了前，桶1的数据已经持久化，桶2判断版本，需要持久化时，腾挪前对桶2持久化，
 也就是发生数据迁移前，对版本号小于全局版本号的桶继续持久化就可以了，
 每个桶只要持久化一次+SAVE命令事务调度，就天然保证分片数据一致性
 对于段也是一样的，段分裂前检查版本号，如果需要，分裂前持久化相关数据
