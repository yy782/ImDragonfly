...
class CompactObj {
 ...
 public:
  ...
  explicit CompactObj(bool is_key) : is_key_{is_key}, tag_{EMPTY} {}
  CompactObj(std::string_view str, bool is_key) : CompactObj(is_key) {
    SetString(str);
  }
  explicit CompactObj(int64_t val) : CompactObj(false) { SetInt(val); }
  explicit CompactObj(const TtlString& ts) : CompactObj(true) { SetTtlStr(ts); }
  ...
  bool operator==(const CompactObj& o) const noexcept {
    ...
    switch (tag_) {
      case EMPTY:
        return true;
      case INT_TAG:
        return u_.ival_ == o.u_.ival_;
      case STR_TAG:
        return u_.str_ == o.u_.str_;
      case TTL_STR_TAG:
        return u_.ttl_ == o.u_.ttl_;
      case ROBJ_TAG:
        return RobjCmp(o);
      default:
...
struct CompactKey : public CompactObj {
  ...
  bool HasExpire() const { return tag_ == TTL_STR_TAG; }
  ...
  bool operator!=(std::string_view sl) const { return !(*this == sl); }
