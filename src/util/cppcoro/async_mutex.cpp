...
cppcoro::async_mutex::async_mutex() noexcept
    : m_state(not_locked), m_waiters(nullptr) {}
...
cppcoro::async_mutex::lock_async() noexcept {
  return async_mutex_lock_operation{*this};
}

cppcoro::async_mutex_scoped_lock_operation
cppcoro::async_mutex::scoped_lock_async() noexcept {
...
void cppcoro::async_mutex::unlock() {
  ...
  if (waitersHead == nullptr) {
    ...
    const bool releasedLock = m_state.compare_exchange_strong(
        oldState, not_locked, std::memory_order_release,
        std::memory_order_relaxed);
    if (releasedLock) {
      return;
    }
...
bool cppcoro::async_mutex_lock_operation::await_suspend(
  ...
  while (true) {
    if (oldState == async_mutex::not_locked) {
      if (m_mutex.m_state.compare_exchange_weak(
              oldState, async_mutex::locked_no_waiters,
