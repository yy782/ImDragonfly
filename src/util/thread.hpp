...
class Thread {
 public:
  ...
  Thread(Thread&& other) noexcept : tid_(other.tid_), joined_(other.joined_) {
    other.tid_ = 0;
    other.joined_ = false;
  }
  ...
  bool joinable() const noexcept { return tid_ != 0 && !joined_; }

  pthread_t native_handle() const { return tid_; }
  ...
  static pthread_t current_tid() { return pthread_self(); }
