...
class CmdArgParser {
 public:
  ...
  std::string_view Peek() { return SafeSV(cur_i_); }
  ...
  auto MapNext(Cases&&... cases) {
    if (cur_i_ >= args_.size()) {
      Report();
      return typename decltype(MapImpl(
          std::string_view(), std::forward<Cases>(cases)...))::value_type{};
    }
  ...
  ::cmn::ArgSlice Tail() const { return args_.subspan(cur_i_); }

  bool HasNext() { return cur_i_ < args_.size() && !error_; }

  bool HasError() const { return error_ == HAS_ERROR; }

  bool HasAtLeast(size_t i) const {
    return !error_ && i <= args_.size() - cur_i_;
  }

  size_t GetCurrentIndex() const { return cur_i_; }
 ...
 private:
  ...
  void NextImpl(Tuple* t) {
    std::get<shift>(*t) =
        Convert<std::tuple_element_t<shift, Tuple>>(cur_i_ + shift);
    if constexpr (constexpr auto next = shift + 1;
                  next < std::tuple_size_v<Tuple>)
      NextImpl<next>(t);
  }
 ...
 private:
  enum ErrorType { NO_ERROR, HAS_ERROR };
  size_t cur_i_ = 0;
  ArgSlice args_;
