...
class ConnectionContext {
 public:
  ...
  const absl::flat_hash_set<std::string_view>& GetWatchKeys() const {
    return watch_keys_;
  }
  bool HasWatchKeys() const { return !watch_keys_.empty(); }
  bool IsDirty() const {
    return watched_dirty_.load(std::memory_order_relaxed);
  }
  bool SetDirty(uint64_t watched_dirty_ver) {
    if (watched_dirty_ver != watched_dirty_ver_.load(std::memory_order_relaxed))
      return false;
...
class CommandContext {
 public:
  ...
  CommandContext(Transaction* transaction, const CommandId* cid)
      : transaction_(transaction), cid_(cid) {}
