...
// KeyLockContext::fps 是 std::span（无 initializer_list 构造），
// 用 LockCtx 承接 braced-list 并提供稳定存储，避免临时对象悬垂。
struct LockCtx {
  std::vector<LockFp> fps;
  KeyLockContext ctx;
  LockCtx(DbIndex db, std::initializer_list<LockFp> list,
          IntentLock::Mode mode = IntentLock::Mode::EXCLUSIVE)
      : fps(list), ctx{db, fps, mode} {}
  operator const KeyLockContext&() const { return ctx; }
};
...
class IntentLockTest : public ::testing::Test {
 protected:
  ...
  bool Acquire(IntentLock::Mode mode, const KeyLockContext& args) {
...
TEST_F(IntentLockTest, AcquireReleaseReacquire) {
  // 每轮都用多 fp 的 KeyLockContext，覆盖各种组合
  LockCtx args_ab(0, {1, 2});
  LockCtx args_abc(0, {10, 20, 30});
  LockCtx args_abcd(0, {100, 200, 300, 400});
...
TEST_F(IntentLockTest, SharedExclusiveMutualExclusion) {
  ...
  // 4 个 fp
  {
    LockCtx args(0, {1000, 2000, 3000, 4000});
    EXPECT_TRUE(Acquire(IntentLock::SHARED, args));
    EXPECT_FALSE(Acquire(IntentLock::EXCLUSIVE, args));
    store_->Release(IntentLock::SHARED, args);
...
TEST_F(IntentLockTest, PartialLockConflict) {
  ...
  {
    LockCtx A_args(0, {1, 2});
    LockCtx B_args(0, {3, 4, 2, 5});  // 冲突在 index=2
  ...
  {
    LockCtx A_args(0, {11, 22});
    LockCtx B_args(0, {33, 44, 11});  // 冲突在 index=2
  ...
  {
    LockCtx A_args(0, {100, 200});
    LockCtx B_args(0, {300, 400, 500});
  ...
  // --- 场景4: 共享锁重叠, 双方都成功 ---
  {
    LockCtx A_args(0, {111, 222, 333});
    LockCtx B_args(0, {333, 444, 555});
  ...
  {
    LockCtx A_args(0, {50, 60});
    LockCtx B_args(0, {70, 80, 90, 60});  // 冲突在 index=3
  ...
  {
    LockCtx A_args(0, {201, 202, 203});
    LockCtx B_args(0, {201, 204, 205});  // 冲突在 index=0
...
TEST_F(IntentLockTest, DifferentFpIsolation) {
  LockCtx args_a(0, {42, 43});
  LockCtx args_b(0, {99, 100});
