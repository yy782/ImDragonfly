...
struct BasicDashPolicy {
  ...
  template <typename U>
  static void DestroyValue(const U&) {}
  template <typename U>
  static void DestroyKey(const U&) {}

  template <typename U, typename V>
  static bool Equal(U&& u, V&& v) {
    return u == v;
  }

  static uint64_t HashFn(uint64_t v) { return std::hash<uint64_t>{}(v); }
};
...
double MeasureTime(Func&& func, const std::string& operation,
  ...
  std::cout << std::setw(25) << operation << ": " << std::fixed
...
void RunPerformanceTest(size_t num_elements) {
  std::cout << "\n=== Performance Test with " << num_elements
            << " elements ===" << std::endl;
  ...
  // Test std::unordered_map
  {
    ...
    size_t node_bytes =
        std_map.size() *
        (sizeof(std::pair<const uint64_t, uint64_t>) + 2 * sizeof(void*));
